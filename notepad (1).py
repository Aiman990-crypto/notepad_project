...
class AdvancedNotepad:
    ...
    def __init__(self, width=800, height=600):
        ...
        self.thisMenuBar.add_cascade(label="About", menu=self.thisHelpMenu)
...
# select_all
    ...
    # About dialog
    def _showAbout(self):
        showinfo("Notepad", "Advanced Notepad with Python and Tkinter by ASHUB,HAMNA,AIMAN,MARYAM")

    # Open a file
    def _openFile(self):
        self._file = filedialog.askopenfilename(defaultextension=".txt",
                                     filetypes=[("All Files", "*.*"),
                                                ("Text Documents", "*.txt")])
        if self._file:
    ...
    def _saveFile(self):
        if not self._file:
            self._file = filedialog.asksaveasfilename(initialfile='Untitled1.txt',
